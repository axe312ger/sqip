...
describe('runPrimitive', () => {
  ...
  test('executes primitive with default config', () => {
    ...
    expect(
      childProcessMock.execSync.mock.calls[0][0].replace(
        VENDOR_DIR,
        '/VENDOR/DIR'
      )
    ).toMatchSnapshot()
    expect(childProcessMock.execSync.mock.calls).toHaveLength(1)
    expect(childProcessMock.execSync.mock.calls[0]).toHaveLength(1)
  })
